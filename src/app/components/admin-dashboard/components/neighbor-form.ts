  ...
  template: `
    <form [formGroup]="form()" (ngSubmit)="submit.emit()" class="space-y-6">
      ...
      <div class="flex gap-4 pt-4">
        ...
        <button type="submit" [disabled]="form().invalid" class="flex-1 px-5 py-4 bg-[#0054a6] dark:bg-[#4a9eff] text-white rounded-2xl font-black uppercase text-xs shadow-xl active:scale-95 transition-all">Guardar Congregación</button>
      </div>
    </form>
  `
})
