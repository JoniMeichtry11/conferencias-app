    ...
    <form [formGroup]="form()" (ngSubmit)="handleFormSubmit($event)" class="space-y-6">
      ...
      @if (form().get('type')?.value !== 'event') {
        <div class="grid grid-cols-1 gap-4">
          <div class="space-y-2">
            <label class="text-[10px] font-black text-[#666666] dark:text-[#999999] uppercase tracking-[0.2em] ml-1">1. Seleccionar Congregación {{ form().get('type')?.value === 'outgoing' ? 'de Destino' : '' }}</label>
        ...
        <div class="space-y-2">
          <label class="text-[10px] font-black text-[#666666] dark:text-[#999999] uppercase tracking-[0.2em] ml-1">3. Seleccionar Tema del Orador</label>
          <select formControlName="conferenceNumber" class="w-full px-5 py-4 bg-[#f8f9fa] dark:bg-[#262626] border-2 border-[#e5e7eb] dark:border-[#333333] rounded-2xl focus:border-[#0054a6] outline-none transition-all text-sm font-bold appearance-none text-[#1a1a1a] dark:text-white">
            ...
            @if (selectedSpeakerRepertoire().length > 0) {
              ...
              <option disabled class="dark:bg-[#1a1a1a]">El orador no tiene temas cargados</option>
            }
          </select>
        </div>
      }
